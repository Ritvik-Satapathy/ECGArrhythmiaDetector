...
ArrhythmiaResult ArrhythmiaDetector::analyze(const vector<int>& rPeaks,
    ...
    for (double rr : rrIntervals) {
        if (rr > avgRR * 1.8 || rr < avgRR * 0.5) {
            extremeCount++;
        }
    }

    double extremeFraction = static_cast<double>(extremeCount) / rrIntervals.size();
    double extremePercent = extremeFraction * 100.0;

    ExpectedHeartRateModel hrModel = expectedHeartRateModel(context);

    // ECG evidence remains dominant. Context changes how the measured heart rate
    // is interpreted, but does not override RR-based evidence from the waveform.
    double irregularityConcern = clamp01((irregularityRatio - 0.05) / 0.30);
    double extremeRRConcern = clamp01(extremeFraction / 0.10);
    double hrConcern = continuousHeartRateConcern(heartRate, hrModel);
    ...
    if (extremeCount > 0) {
        reason << extremePercent
               << "% of RR intervals were much longer or shorter than the average. ";
    } else {
        reason << "No extreme RR intervals were found by the current rule. ";
    }
